    ...
    <div class="content-body">
        <div class="row page-titles mx-0">
            ...
            <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="javascript:void(0)"> {{ __('site.brands') }}</a></li>
                </ol>
            </div>
        </div>
        <!-- row -->
        <div class="container-fluid">
            <div class="row justify-content-between mb-3">
                <div class="col-12 ">
                    <h2 class="page-heading">{{ __('site.list') }} {{ __('site.brands') }}</h2>
                    <p class="mb-0">{{ __('site.count') }} {{ __('site.brands') }} : {{ $brands->total() }}</p>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                        ...
                        <div class="card-body">
                            <div class="card-header row">
                                <div class="col-md-6">
                                    ...
                                    <form action="{{ route('dashboard.brands.destroy', 'delete') }}" method="post"
                        ...
                        <div class="card-footer">
                            <nav>
                                <ul class="pagination pagination-sm pagination-style-2">
                                    ...
                                    @endphp
                                    <li
                                        class="page-item page-indicator {{ $brands->onFirstPage() ? 'disabled' : '' }}">
                                        <a class="page-link" href="{{ $brands->previousPageUrl() . $append_url }}">
                                            <i class="icon-arrow-left"></i></a>
                                    </li>
                                    @foreach ($brands->getUrlRange(1, $brands->lastPage()) as $num_page => $url_page)
                                        <li
                                            class="page-item {{ $num_page === $brands->currentPage() ? 'active' : '' }}">
                                            <a class="page-link"
                                                href="{{ $url_page . $append_url }}">{{ $num_page }}</a>
                                        </li>
                                    @endforeach
                                    <li
                                        class="page-item page-indicator {{ $brands->hasMorePages() ? '' : 'disabled' }}">
                                        <a class="page-link " href="{{ $brands->nextPageUrl() . $append_url }}">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('site.filter') }} {{ __('site.brands') }}</h5>
                    <button type="button" class="close" data-dismiss="modal"><span>×</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('dashboard.brands.index') }}" method="GET" id="filter-form">
